...
产物侧的结论是"片数由底物决定",反应物侧是它的镜像:**输入的分子数同样不是模板的
性质**。位置式契约("N 个模板 ↔ N 个分子,第 i 个配第 i 个")把这件事写死了,于是
分子内反应 —— 两个片段落在同一个分子上 —— 根本无从表达。

`run_on_substrate` 把输入拼成一张图,各片段自由找位置、只要求两两不重叠。USPTO-50k
上位置式跑不了的 301 条,它命中 **291 条(96.7%)**;RDKit 与 rdchiral 都是 0
(两者都抛 "Number of reactants provided does not match")。详见
[基准 README](../README.md#位置式契约跑不了的-301-条换一张图就能跑)。
