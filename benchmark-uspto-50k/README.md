...
1286 条未命中**逐条归了因**(见[归因](#未命中逐条归因)):全部是立体化学的差异,
...
|---|---|---|---|---|---|
| omgkit 正向 | 21.0 µs | 47.2 µs | **352 µs** | **5.5 ms** | **1.72 s** |
| RDKit 正向 | **15.4 µs** | 64.3 µs | 642 µs | 10.4 ms | 2.23 s |
| omgkit 逆向 | 14.3 µs | **27.5 µs** | **66 µs** | **0.69 ms** | **0.85 s** |
| RDKit 逆向 | **13.3 µs** | 34.4 µs | 121 µs | 1.49 ms | 1.00 s |

RDKit 的中位数更小、omgkit 的尾部更短:p99 上 omgkit 快 1.83 / 1.84 倍,最大值快
1.90 / 2.15 倍,跑完五万条 omgkit 省 22.9%(正向)与 15.3%(逆向)。原因在下面
[怎么读这张图](#怎么读这张图)里。比值一律从 `results/summary.json` 现算,别手抄 ——
上一版这里写的 "21%" 与 "1.9 倍" 就是抄错的。
...
1286 条未命中(正向 676、逆向 610)**全部**归了因。先逐个立体中心定责,再把整条
反应归到最严重的那一档:
...
| **方向键的键级要求比默认键还严** —— 芳香键上写方向就一处也匹配不上,见[下下节](#方向键写在芳香键上模板就一处也匹配不上) | 逆向命中 49400 → **49402**;归因的 2 条缺口补上 |
...
## 方向键写在芳香键上,模板就一处也匹配不上

SMARTS 里**不写**键符号表示"单键或芳香键"。而 `/` `\` 的键级判据一度写成了
**仅单键** —— 比不写还严。于是模板把方向落在一根芳香键上时,整条模板一处也匹配
不上,`run` 交白卷:

| 查询 | omgkit(修前) | omgkit(修后) | RDKit |
|---|---|---|---|
| `[#16;a]/[c]` | **0** | 2 | 2 |
| `[#16;a][c]` | 2 | 2 | 2 |
| `[#16;a]-[c]` | 0 | 0 | 0 |

第二行与第一行必须同数 —— **加个方向只该改朝向,不该改键级**;第三行说明放宽的
只是芳香那一档,写死单键仍然落空。

语料上是 2 条,US08841334B2(row 15318)与 US07683084B2(row 31347),都是
2-亚氨基苯并噻唑的 N-酰化逆向。模板由 rdchiral 抽出,它把方向写在了环内的 S—c
键上;而记录里的方向写在 n—c 键上 —— **两端参照原子分处双键两侧**。换算过来几何
其实一致(用 RDKit 开 `useChirality=True` 独立验过,也匹配),omgkit 却在更前面的
键级那一关就否掉了。

修完两条都命中,逆向 49400 → **49402**;逐条对拍 2 条变化**全部**是"未中 → 命中",
0 条被碰坏。判据是 `a_direction_written_on_an_aromatic_bond_still_matches`,
撤修即红,并且卡住了另一头:把模板要的几何翻过来必须 0 处命中,免得"放宽键级"
变成"顺反不判了"。

### 归因脚本自己有个洞

这 2 条是**查归因表加不齐**才露出来的:未命中 1288 条,而逐中心定责只出 1286 条。
差的 2 条 `omgkit_preds` 是空的 —— 定责脚本要拿预测与真值逐中心比,没有预测就
无从比起,于是被静默跳过。

**而"一个产物都没有"恰恰是最重的一档。** 归因表因此看着干净:它只统计了能比的那些。
修完之后 1286 条未命中、1286 条归了因,缺口为 0。

教训与上一节同源:**判据要能报告自己没覆盖到什么**。现在 `summarize.py` 的
归因合计与未命中数对不上就会露馅,这一条要一直对得上。
