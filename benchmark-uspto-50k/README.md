...
| **产物净化不过** | **0** | 279 | 不对称 |
| 丢掉整个不连通组分 | 6 | 6 | 相同 |
| 模板删原子导致的孤立 | 1487 | 1487 | 相同 |

**看"相同"还是"不对称"是这张表的用法。** 两个引擎数字一字不差的那些档,成因
必在模板或约定 —— 实现差异撞不出相同的数。后两档就是这样:前者是"搬运从匹配到
的原子出发遍历,完全不连通的组分走不到"这条已知约定;后者是模板删掉某个原子时,
挂在它身上又没有别的路连回保留部分的原子一并失去落脚点(叔丁酯水解是标准例子,
模板删 2 个而实际少 4 个)。真正要看的是**不对称**的四档,全部指向同一侧。
...
连通分量数则两类同时正确。盐把同一个缺口暴露在反应物侧(语料上占 7.2%)。
...
scripts/   抽取、基准、归因、对拍、出图,共 21 个脚本
