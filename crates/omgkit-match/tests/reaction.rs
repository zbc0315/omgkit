...
/// 参照原子走了、**而且没人顶它的槽位**:改参照到另一侧,并把顺反翻过来。
///
/// 上一条守的是"顶替者占住了槽位"那一路(醚氧换成碘)。这一条是另一路:
/// 走掉的位置由**隐式氢**补上,而隐式氢没有下标、做不了参照原子。
/// 先前这一路直接放弃,整根双键的顺反跟着作废 —— 而双键还在、两端各自
/// 还有取代基,构型依旧成立。
///
/// # 例子取自真实语料,期望值取自**真实三维构象**
///
/// `large.smi` 里的三个硝基烯烃,`[C:1][N:2]>>[C:1].[N:2]` 打掉硝基。
/// 期望值不是抄外部实现的,也不是本实现自己算的:把底物嵌成真实三维构象、
/// 把离去的氮**原地**换成氢再读回构型,五个 seed 一致 —— 而且那条判据先
/// 自校准过(同一条路读底物本身,五个 seed 都还原输入的构型)。
///
/// 外部实现在这三条上与几何一致,与本实现先前的输出不一致。
#[test]
fn bond_stereo_rebases_to_the_other_side_when_nothing_fills_the_slot() {
    const RXN: &str = "[C:1][N:2]>>[C:1].[N:2]";
    // (底物, 该出现的那个烯烃产物)
    const CASES: &[(&str, &str)] = &[
        (r"[N+](/C(=C/C1C=CC=CC=1)C)([O-])=O", r"C/C=C\c1ccccc1"),
        (r"[N+](/C(=C/C1SC=CC=1)C)([O-])=O", r"C/C=C\c1cccs1"),
        (r"[N+](\C(=C/C1=CSC=C1)C)([O-])=O", r"C/C=C/c1ccsc1"),
    ];
    for (substrate, want) in CASES {
        let got = flat(RXN, &[substrate]);
        let want = canonical(want);
        assert!(
            got.iter().any(|p| p.split('.').any(|f| f == want)),
            "{substrate}:该得到 {want},实得 {got:?} —— \
             参照原子走了而没人顶槽位时,顺反不该整个作废"
        );
    }
    // **不许两个几何塌成一个。** 上面三条各自只看自己,若实现把顺反一律抹平,
    // 每一条都会退化成同一个无立体的串 —— 那时上面就全是空过的。
    // 头两条是同一个几何、第三条是反过来的,拿它们互核。
    let a = flat(RXN, &[CASES[0].0]);
    let b = flat(RXN, &[CASES[2].0]);
    let strip =
        |v: &Vec<String>| -> Vec<String> { v.iter().map(|s| s.replace(['/', '\\'], "")).collect() };
    assert_ne!(a, b, "一对相反的几何给出了同一批产物");
    assert_eq!(
        strip(&a).len(),
        strip(&b).len(),
        "抹掉斜杠之后条数该一样 —— 不一样说明差别不只在几何上"
    );
}
