...
//! ## 净化之外:两个**读**它结果的模块
//!
//! 本 crate 里还有三个模块**不是净化的步骤**:[`gasteiger`]、[`descriptors`]
//! 是净化输出的消费者,[`explicit_hs`] 是一次独立的图编辑(与 `remove_hs` 一对)。
//! 前两个放在这里
...
//! | [`descriptors`] | 图神经网络特征化要读的原子/键描述符(12 + 7 个字段),汇到一处交付 |
//! | [`explicit_hs`] | 把隐式氢补成真原子。**改原子数**,所以不在那 12 步里 —— 与 [`mod@remove_hs`] 是一对 |
//!
