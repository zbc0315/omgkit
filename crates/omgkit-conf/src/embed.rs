...
//!
//! 它要成立,前提是 `U` 能在三维里精确摆出来。它不能:实测 `fit3 = 0.930`,
//! 前三个特征值只占正谱质量的 93%,剩下 7% 是被**截断**掉的。
//! 而从高维往低维投影**恰恰是制造交叉的操作** —— 四维里不打结的曲线投到三维
//! 就会冒出交叉点。所以被压掉的那 7% 谱质量,压出来的正是自穿。
//!
//! 实测也不支持"起点很好":嵌出来的坐标里越界 >0.1 Å 的原子对占 **29.3%**。
//! `U` 比"区间内随机取"好(那个是 36.7%),但好得有限,而且**好在哪一头还没量** ——
//! 判据现在把"撞在一起"和"拉太开"用 `max` 合成了一个数,方向丢了。
//!
//! # 不合格时怎么办:**降级,不是失败**
//!
