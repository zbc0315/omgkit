...
//! # 第二条是**三配位立体中心唯一的手性力**
//!
//! 实测三栏(`smoke.chirality.jsonl`,248 个中心,其中 8 个三配位):
//!
//! | | 精修步数 | 1-2 键 | 1-3 角 | 真值口径手性 |
//! |---|---|---|---|---|
//! | 手性项**全关** | 188 | 0.0% | 0.4% | **64.5%**(160/248) |
//! | 只有四配体项(`WEIGHT_UMBRELLA = 0`) | 181 | 0.0% | 0.1% | **98.4%**(244/248) |
//! | **两条都有** | 179 | 0.0% | 0.1% | **100%**(248/248) |
//!
//! **第二行掉的那 4 个中心全是三配位**(亚砜的 S、膦的 P):
//! `[S@@](=O)`、`[P@H]`、`[P@@H]`。原因在结构里,不在语料里 ——
//! 四配体行列式要**四个**配体,三配位那一档根本组不出来
//! (见本文件里 `if !c.is_three_coordinate()` 那一支),于是中心基点项
//! 是它们**唯一**的手性力。清零 = 交付随机构型。
//! `smoke.lonepair.jsonl` 上同一个消融掉得更狠:21 → 15(71.4%)。
//!
//! # 这张表先前说的是反话,而说反话的原因是基准脱钩
//!
//! 上一版写的是"第二行与第三行分不出来 …… 中心基点这一项在当前语料上
//! 一个中心都没救到",留着它的理由只能诉诸"代价是零、堵的洞是真的"。
//!
//! **那不是语料的性质,是基准掉了东西。** `smoke.chirality.jsonl` 与生成它的
//! `dump_chirality.py` 脱钩了四个月(提交 `61b8d58` 教会脚本收三配位立体中心,
//! 却没重导基准),入库那份里 247 个中心**全是四配位** —— 而四配位正是这一项
//! 唯一使不上劲的那一档。消融当然分不出来:能分出来的分子被基准弄丢了。
//!
//! 重导之后同一个消融当场分得出来。这一条的教训不是"数错了",是
//! **消融报"没区别"时,先问它有没有拿到能出区别的那批数据**
//! (闸见 `harness/check_baseline_schema.py` 与 `tests/baseline_sizes.rs`)。
//!
//! 头一版这张表还犯过另一个错:第二行标的是"只有四配体项 … 64.8%",
//! 而当时代码里只剩中心基点一项,把权重清零关掉的是**全部**手性项。
//! 独立审核复现时拆穿了这一点。
//!
//! 另外那一档灾难也照旧成立:四配体行列式对伞形翻转**在数学上就是瞎的**,
//! 给 300 个分子各翻一个中心的伞,外部判据 `verify_stereo.py`
//! 从 290/301 掉到 **2/301**;判据 `手性项挡得住伞形翻转` 钉着它。
//!
...
///
/// 实测 240 个真实构象上的**四配位**中心(基准里另外 8 个是三配位,
/// 组不出四配体行列式):`|V_配体|` 最小 5.882、中位 8.389、最大 16.446,
