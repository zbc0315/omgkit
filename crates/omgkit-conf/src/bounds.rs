...
///
/// **本引擎当前不写 1-5 约束**(见 `Stats` 的 `n15` 字段),所以这个常数眼下没有调用方。
/// 留着是为了让那一档重新接回来时不必再去翻上游的行号。
...
/// 逐环沿圈取连续四元组记 `|扭转角|`,按 **环尺寸 + 是否芳环 + 是否全 sp³**
/// 分桶,取 `p05`/`p95`。
///
/// # 拍出来的数错在哪
///
/// 头一版这几个数是我按化学常识写的,实测下来小环还行、**大环错得离谱**
/// (下表取每个尺寸里非芳桶的最大 p95,值来自当前的 `mmff.ringtorsion.tsv`;
/// 先前这张表是**加上"全 sp³"这一维之前**量的,一直没跟着重导):
///
/// | 环 | 我拍的上界 | 实测 p95 |
/// |---|---|---|
/// | 4 元 | 25 | 34.9 |
...
/// 芳环也不是精确的 0(五元 p95 = 2.8、六元 2.2)。
///
/// 表里没有的尺寸(11~15、17~27、29 以上)按"越大越柔"退到全程 —— 大环本来就柔,
/// 硬给一个内插值是在编数。
#[must_use]
pub fn ring_internal_torsion(size: usize, aromatic: bool, sp3: bool) -> (f64, f64) {
    ...
    // **芳环钉中位那一个值,饱和环给 [p05, p95] 区间** —— 分档的理由见下面两支
    // 里的注释,那里连着实测代价一起记着。先前这里写的是"一律取中位,不取
    // p05/p95",而十几行之下的代码就在返回区间;那句还引了"饱和六元环中位
    // 20.9°",而分桶加上"全 sp³"这一维之后同一桶的中位是 53.8°。两处都是旧的。
    //
    // 表里没有的尺寸(11~15、17~27、29 以上)退到全程 —— 表里实有的是
    // 3 4 5 6 7 8 9 10 16 28,大环本来就柔,内插是在编数。
...
/// 环感知的结果打成一包 —— 三张按环编号索引的表,一起传比三个参数少出错。
struct Rings<'a> {
    sets: &'a [Vec<u32>],
    aromatic: &'a [bool],
    sp3: &'a [bool],
...
///
/// **第 2、3 行只在环外那一端挂在 sp² 环原子上时成立**,理由见函数体里那段
/// (sp³ 环原子有两个朝外的邻居,扭转是析取而不是值)。
///
/// 不成立就返回 `None`,由调用方另找退路。**注意那条退路不一定更松**:
/// 它走的是 `torsion_envelope(shared_ring(&[i, k, l, j]), …)`,用的是**含全部四个
/// 原子**的最小环 —— 桥环里那往往是**另一个**环,而包络表对 3/4 元给 `(0,0)`、
/// 5 元给 `(0,40)`、6 元给 `(0,60)`,可以比被否决的那个区间更紧。
/// 实测(hard + bridged + large 前 1500,共 573854 对):变松 8673 对、**变紧 79 对**,
/// 分布在 20 个桥环分子上(金刚烷、奎宁环、莨菪烷、甾体、青蒿素……)。
/// 拿 RDKit 构象回量,这 79 对**收紧的方向是对的**:同一批分子上最狠的 1-4 越界
/// 从 0.984 Å 降到 0.207 Å。所以结果没问题,但"只会变松"这个说法是错的,
/// 别拿它当安全性论证。
///
...
fn ring_path_torsion(
    ...
    // possible",给顺式到反式的全程 —— 宁可界松也不要摆不出来。
    //
    // **钉住的是三桶:芳环、全 sp³ 环、以及下面那条共轭平面段。**
    // 先前这里写的是"芳环才钉,饱和环退回全程",而下一行就说的是另一回事,
    // 代码也不是那么做的 —— 新段落追加在了旧段落前面,旧的没删。
    ...
    let set = &ring_sets[r];
    let has = |a: u32| set.binary_search(&a).is_ok();
    // **环外那一端,只有挂在 sp² 环原子上时扭转才是确定的。**
    //
    // 上面那张表的第 2、3 行("都在环外"、"一个在环外")默认每个环原子只有
    // **一个**朝外的取代基,躺在环平面里。sp² 成立(苯上的 H),**sp³ 不成立**:
    // 它有**两个**朝外的邻居,分居环平面上下,绕 `k–l` 轴差约 120°。
    // 把两者一起钉到同一个 `τ`,等于宣称一个不存在的构型。
    //
    // 几何上:`k` 是 sp² 时,`i`、`k`、`l`、`k` 的另一个环邻居 `k′` 共面,于是
    // `torsion(i-k-l-j) = 180° − torsion(k′-k-l-j)` **精确成立**,这正是那张表。
    // `k` 是 sp³ 时右边是个三值析取,取其一就是编。
    //
    // # 这不是理论顾虑
    //
    // 环丙烷 `C1CC1`:`H–C–C–H` 真实距离顺式 **2.540 Å**、反式 **3.133 Å**,
    // 而这里对**两者**都给 `[2.465, 2.585]`(τ=0 钉死,宽 0.12)。反式那一对被
    // 摁下 0.6 Å,整个分子跟着缩:C–H 键实测 **0.964 Å**(界 `[1.084, 1.104]`)。
    // 立方烷、四面体烷、棱晶烷同理,`hard.smi` 上 **19.1%** 的分子至少断一根键。
    //
    // 顺带堵掉一个量法产物:`ring_internal_torsion` 的三元环那一行是 `(0, 0)`,
    // 计数 255 —— 可三元环**没有**环内四原子路径,`measure_ring_torsion.py` 绕圈
    // 取四元组时首尾撞在一起,量出来的 0 是量法的产物。全 sp³ 三元环的每一条
    // 1-4 都是环外路径,这条守卫一并把它挡住了。
    //
    // # `has(end)` 那一半没有判据护着 —— 因为它护的东西量不出价值
    //
    // 变异验证:把 `has(end) ||` 去掉(于是**环内**路径也要求 sp²,饱和环的
    // 环内扭转不再钉),**`feasibility` 的每一道闸照样绿**。逐行对完整输出:
    //
    // | | 原版 | 去掉 `has(end)` |
    // |---|---|---|
    // | 界宽 p90 | 6.407 | 6.402 |
    // | 1-4 钉住率 | 60.4% | 60.4% |
    // | `large` 断键分子 | 30 | **28** |
    // | `large` 长程越界 | 8126 | **8355** |
    // | `hard` 1-4 越界 | 104 | **105** |
    //
    // 有升有降,全在噪声里 —— 也就是说**饱和环那条"环内扭转钉 [p05, p95]"
    // 在任何一条判据上都量不出价值**。这不是补一条测试能关掉的缺口:补一条
    // 只为护住它的判据,就是在编判据。所以这里如实记下,行为不动 ——
    // 要拆是另一件事,不该和一条正确性修复捆在同一次提交里。
    let sp2_or_in_ring = |end: u32, attach: u32| has(end) || sp2(attach);
    if !sp2_or_in_ring(i, k) || !sp2_or_in_ring(j, l) {
        return None;
    }
...
pub struct Stats {
    ...
    /// **恒为 0 —— 1-5 链式约束试过,撤了。**
    ///
    /// 想法是:一条 5 原子路径上两个扭转都被钉住时整段几何就定死了,而三角
    /// 光滑化推不出这一点(它只知道两两的界,不知道两个扭转是联动的)。撤掉的
    /// 理由记在本文件"1-5 链式约束:试过,撤了"那一节。
    ///
    /// 字段留着是为了让"这一档现在是 0"说得出口 —— 删掉它,调用方会以为这个
    /// 引擎本来就没有 1-5 这个概念;留着而不说明,调用方会把 0 读成"这个分子
    /// 没有可用的 1-5 路径"。两种误读都比一句话贵。
...
/// # 前置条件 —— 调用方负责
///
/// 分子应当**已经补过氢**,并且**已经把 SMILES 的 `/` `\` 折算成双键自己的
/// `BondStereo`**(`omgkit_io::stereo::perceive_bond_stereo`)。后者漏了的话
/// `stereo_path_torsion` ——1-4 扭转最硬的那条来源、给的是确定值不是区间——
/// **一次都不会发力**,而界照样建得出来:静默地松。
/// 实测整条构型流水线先前压根没做这一步,全语料 405 条双键受影响,
/// 外部判据上 10 个分子交付的几何站错了边。
///
/// 这一条不靠文档守:`omgkit_io::stereo::directions_not_perceived` 是配套的谓词
/// (与感知**由构造保证一致**),`examples/feasibility.rs` 拿它当闸。
/// 这里不自己再判一遍 —— 判"该不该折算"要用规范秩分辨等价取代基,
/// 那是 `omgkit-io` 的活;在这儿写第二份实现,两边迟早分岔。
/// (确实分岔过一次:头一版在这儿数"两端都有方向键",于是
/// `F/C=C(\F)F` 这类"方向本就是噪声"的分子被报成漏调,而且修不绿。)
#[must_use]
pub fn build(mol: &MolBuilder) -> (Bounds, Stats) {
    ...
    for (bidx, bd) in mol.bonds().iter().enumerate() {
        ...
        for &i in &nk {
            for &j in &nl {
                ...
                // 芳环上的扭转是**确定值**(见 `ring_path_torsion`),不是区间;
                // 解不掉的才退回按环尺寸的包络
    ...
    // 也就是 8 个分子**完全嵌不出来**。
    //
    // 覆盖率是这个项目的头号指标(整件事就是为了赢 RDKit 那条失败率的线 ——
    // 2025.09.2 实测 0.41%,见 `harness/baseline_rdkit_etkdg.py`),
...
mod tests {
    ...
    /// **三元环上任意两个重原子既成键、又同为第三个原子的 1-3。**
    ///
    /// 直接量到的键长比"两根键 + 夹角推出来的距离"可靠得多,所以 1-3 不许往
    /// 已经成键的那一对上写 —— 让角推的去挤键,是拿差的信息盖好的。
    #[test]
    ...
    /// 两者都是确定的,不是"顺式到反式"的区间。这条盯住 `ring_path_torsion`
