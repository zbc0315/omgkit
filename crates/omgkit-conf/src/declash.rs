...
///
/// 轮数上限**不是**可调的旋钮:把它放到 400,全语料的结果与放到 32 逐位相同 ——
/// 算法自己会停(本地实测最多 13 轮)。
///
/// # 为什么是 32 而不是 16
///
/// 同一份代码、同一份语料,**macOS aarch64 上最多 13 轮,Linux x86_64 的 CI 上 15 轮**。
/// 差别来自 `sin_cos` 这类 libm 函数最后几位的跨平台差异:近平局时选中了不同的
/// 候选角,于是走了不同的搜索路径。**结果质量一样** —— 两边的地板 1.489、
/// p05 1.810、中位 2.052、低于 1.6 Å 的分子 3 个,逐位相同;不同的只有
/// "接受转动 3546 / 3564"和"13 轮 / 15 轮"这类路径上的数。
///
/// 跨平台逐位一致**不是这个 crate 的目标**(那是 v1 付过大代价的路)。
/// 但上限得留够余量:16 对 CI 的 15 只剩一轮,换台机器就可能顶上去。
