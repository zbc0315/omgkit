...
/// # 这个数决定了要不要上四维
///
/// 实测(248 个中心):嵌完直接对的 **51.6%** —— 基本就是掷硬币,
/// 与"嵌入给出的坐标系定向是任意的"完全吻合。做一次全局反射之后 **85.1%**。
///
/// 剩下的 14.9% 是**个别中心相对多数错**,全局反射按定义救不了它们。
/// 但那一档**三维精修救得了**:翻一个中心只要它自己的手性体积过零,
/// 是局部的、有限的势垒;而全局反射要求**所有**中心同时压平 ——
/// 两者的势垒不是一个量级。所以四维先不做,等精修落地之后量剩下多少再说。
///
/// 闸设在 0.80,是贴着现值 0.851 的棘轮:全局反射那段逻辑一旦退化,
/// 这个数会掉回 0.52 附近,当场就红。
...
/// 248 变成 244,漏 0、错 0,**退 0**。`conformer_oracle` 已经有这条对账
/// (`truth_declared`),这里补齐 —— 补上之后同一个变异退 1。
...
fn main() {
    ...
    // **顺反列必须在。** 理由与 `conformer_oracle` 里那条相同:列没了,
    // 带 E/Z 的分子会被当成没有顺反的分子建界,而这条判官看不出区别。
    if !has_stereo_col {
        eprintln!(
            "\n这份基准的键元组**没有顺反列** —— 带 E/Z 的分子进来会被当成没有\n\
             顺反的分子。拿当前的 dump 脚本重导基准(见 shared/baseline_mol.rs)。"
        );
        fatal = true;
    }
