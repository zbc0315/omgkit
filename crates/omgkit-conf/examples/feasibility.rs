...
//! 是整个项目的**头号指标**:要赢的正是 RDKit 的失败率 —— **0.41%**
//! (ETKDGv3,RDKit 2025.09.2,`harness/baseline_rdkit_etkdg.py` 量的)。
//! 这个数跟版本走,引用它的地方都要连版本一起写。
...
/// # 闸为什么必须比目标严
///
/// 要赢的是 RDKit ETKDGv3 的失败率;界不可行的分子连嵌入都进不去,直接计入
/// 失败。所以这一项只要碰到那条线,算法在还没开始的地方就已经输了。
///
/// **参照的数跟 RDKit 版本走,而且是量出来的。** 同一份 `large.smi`、
/// 仓库钉的 2025.09.2:嵌入失败 36 个,**0.41%**(2026-08-26 实测,
/// `python3 harness/baseline_rdkit_etkdg.py`)。先前这里引的 0.52% 出自更早的
/// 版本,而那个脚本的语料路径写死成一个早已删掉的 worktree ——
/// **参照量不出来就只是一句传说**,所以路径改成了命令行参数。
///
/// 先前这条闸在 `bounds_oracle` 里定的是 **2%** —— 比目标松了近四倍,
/// 于是判据一路是绿的,而实测 0.65% 早已越过目标。**闸松于目标等于没有闸。**
///
/// 一路降下来:0.65% → 0.54%(修了角的回退顺序)→ 0.34%(修了 1-3 的写入规则)
/// → 0.06%(1-4 的五个距离改用中点而非上限)→ 现值 **0.01%**(8831 个里 1 个)。
///
/// **棘轮跟着现值走,不然它会悄悄松掉。** 这条闸原本设在 0.12%(约 10 个分子),
/// 理由是"贴着现值 5 个分子留一倍余量";而现值后来降到 1 个,注释没跟着改,
/// 那道闸就从"一倍余量"变成了**十倍余量** —— 一个 10 倍的退化能悄悄溜过去。
/// 现在设在 0.04%(3 个分子),仍是现值的 3 倍:这个数是确定性的(同一份语料
/// 跑两遍逐位相同),留 3 倍是给跨架构浮点差与新加语料的余地。
///
/// 另外这条闸放在**全语料 8831 个分子**上,不放在 400 个分子的判官里:
/// 400 个样本上真实率 0.34% 只对应 1.4 个分子,泊松噪声足以让闸随机红绿。
const MAX_INFEASIBLE_FRAC: f64 = 0.0004;
...
/// 四档越界的**逐语料棘轮**:每份语料记下自己的现值(占比,%),闸取它的
/// [`RATCHET`] 倍。下标是 1-2 / 1-3 / 1-4 / 长程。
///
/// # 为什么必须逐语料,不能一个常量管两份
///
/// 先前 1-2 与 1-3 各是一个全局常量,同时管 `large.smi` 与 `hard.smi`。
/// 那样的闸**只能跟最差的那份语料一样紧,在另一份上就是瞎的** ——
/// 1-3 的闸 1.5% 对 `hard` 的 0.818% 是 1.8 倍,对 `large` 的 0.148% 却是 10 倍。
///
/// 变异实测(把精修迭代从 400 砍到 60,一个很重的缺陷):
///
/// | | 1-2 键 | 1-3 角 | 1-4 扭转 | 长程 |
/// |---|---|---|---|---|
/// | `large` 现值 | 0.026% | 0.148% | 0.353% | 0.257% |
/// | `large` 变异后 | 0.155% | 1.005% | 0.956% | **1.444%** |
/// | 旧的全局闸 | 0.15% | 1.5% | (没有) | (没有) |
/// | 会不会红 | 勉强(1.03 倍) | **不红** | **不红** | — |
/// | `hard` 变异后 | 0.074% | 1.186% | 5.196% | 0.215% |
/// | 旧闸会不会红 | 不红 | 不红 | — | — |
///
/// 也就是说:一个能让全语料几何明显变差的缺陷,旧闸**只在一档上勉强红一次**。
/// 换成逐语料 2 倍棘轮之后,`large` 那侧**四档全红**。
///
/// # 1-4 与长程先前根本没有闸
///
/// 这两档一直只打印不设闸,而**报告里最大的那个越界率恰恰是 1-4**
/// (`hard` 上 3.335%,是 1-3 的四倍)。只打印不设闸的数读起来像"守住了"。
/// 长程那一档还有个额外的用处:它多半是自穿的前兆,而自穿那两条闸只看
/// "穿没穿过去",看不见"快穿了"。
///
/// # 新语料怎么办
///
...
const RATCHET: f64 = 2.0;

/// 每一档至少容忍几对。对数少的语料上按比例设闸会被量化噪声抖红 ——
/// `hard.smi` 的 1-2 档一共才 1349 对,现值 1 对,2 倍就是 2 对,
/// 跨架构浮点差翻一对就红。与 [`MIN_BROKEN_ALLOWANCE`] 同一个理由。
const MIN_VIOL_ALLOWANCE: u64 = 3;

/// 语料不在 [`PINNED`] 里时的粗闸。**这一组就是闸本身,不再乘 [`RATCHET`]** ——
/// 它是"没人钉过的语料"的兜底,不是某份语料的现值。
const FALLBACK: [f64; 4] = [0.15, 1.5, 7.0, 0.5];
...
fn main() {
    ...
    println!(
        "  参照 RDKit ETKDGv3 2025.09.2:large.smi 0.41%、hard.smi 0.00% \
         (harness/baseline_rdkit_etkdg.py 量的)。这一行必须低于参照 —— \
         难例那一档参照已经是 0,我们也只能是 0"
    );
    ...
    if pi / 100.0 > MAX_INFEASIBLE_FRAC {
        eprintln!(
            "\n界不可行 {pi:.2}% > {:.2}% —— 这是头号指标,要赢的是 RDKit 的 0.41%",
    ...
    if pinned.is_none() {
        println!(
            "  ⚠ 语料 `{stem}` 不在 PINNED 里,四档几何回落到粗闸 {FALLBACK:?}% —— \
             把它的现值钉进 PINNED 才有棘轮"
        );
    }
    // 钉住的那份乘棘轮;回落那组本身就是闸
    let limits = pinned.map_or(FALLBACK, |v| v.map(|x| x * RATCHET));
    for (k, (name, hint)) in [
        ("1-2 键", " —— 最硬的一档,产物在化学上就是错的"),
        ("1-3 角", ""),
        ("1-4 扭转", ""),
        (
            "长程",
            " —— 这一档多半是自穿的前兆,而自穿那两条闸只看穿没穿过去,看不见快穿了",
        ),
    ]
    .into_iter()
    .enumerate()
    {
        let (got, total) = viol[k + 1];
        #[allow(
            clippy::cast_precision_loss,
            clippy::cast_possible_truncation,
            clippy::cast_sign_loss
        )]
        let cap = ((total as f64 * limits[k] / 100.0).ceil() as u64).max(MIN_VIOL_ALLOWANCE);
        if got > cap {
            eprintln!(
                "\n{name}越界 {got} 对 > {cap} 对({:.3}% > 闸 {:.3}%){hint}",
                pct(got, total),
                limits[k]
            );
            fatal = true;
    ...
    println!(
        "\n十三条都过:空区间 / 界不可行 / 顺反没折算 / 原子重合 / 非有限数 / 该出没出 / \
         1-2 键 / 1-3 角 / 1-4 扭转 / 长程 / 断键分子 / 键交叉分子 / 键没断却穿环的分子。"
    );
}
