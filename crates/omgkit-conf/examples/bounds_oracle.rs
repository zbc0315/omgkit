...
/// 那件事的镜像,只不过这次藏住的是**退步**。
///
/// 定在 1.20:全量档实测 1.06、冒烟档 1.17(小样本更松,是它卡着这个数)。
/// 1-5 链式约束落地后这个数要跟着降。
...
/// 1.599 → 1.313 → 1.269 → 1.162 → 1.043 → 1.020 → **1.004**,四档里三档逐位相同:
///
/// | 档 | 我们 | RDKit | 比 |
/// |---|---|---|---|
...
// ⚠ **这条闸是被放松过的,那是一笔记着账的退步,不是新常态。**
//
// 1.004 → 1.031,原因是饱和环的 1-4 扭转从"钉住一个值"退回了顺式到反式全程。
// 而那一步是**必须**的:表里的中位扭转(六元环 20.9°)与同表的键角(111.6°)
// 几何上不相容,拼在一起是摆不出来的构型 —— 实测端到端,无环分子精修到恰好零、
// 饱和环全线崩(环己烷 1-4 越界 94.1%)。
//
// 换来的:
//
// | | 之前 | 之后 |
// |---|---|---|
// | 界不可行(头号指标) | 0.06% | **0.01%** |
// | 判据一(真实构象越界) | 0.863% | **0.135%** |
// | 端到端 1-2/1-3/1-4 越界 | 7.3 / 21.3 / 33.4% | **0.0 / 0.1 / 0.0%** |
// | 1-4 界宽比 | 1.00 | **7.68** ← 代价在这里 |
//
// **原先以为的修法试过了,不管用 —— 记下来免得有人再试一遍。**
//
// 那个猜想是:把 `ring_internal_torsion` 的分桶扩到"是否全 sp³",饱和环就能
// 重新钉住。**分桶这一层完全对上了**:表已重跑,六元全 sp³ 的中位是 **53.8°**,
// 与闭环条件(`cos τ = −cos θ/(1+cos θ)`,键角 111.6°)解出的 **54.4°**
// 只差 0.6°;先前那个 20.9° 正是两个总体被揉在一起的中位。
//
// 但收益几乎没有:
//
// | 做法 | 1-4 界宽比 | 判据一 | 端到端 |
// |---|---|---|---|
// | 全钉(原始) | 1.00 | 0.863% | **崩**(环己烷 1-4 越界 94.1%) |
// | 饱和环不钉 | 7.68 | **0.135%** | 完美 |
// | 全 sp³ 钉中位 | 7.54 | 0.431% | 完美 |
// | 全 sp³ 用 p05–p95 | 7.54 | 0.353% | 完美 |
//
// 三种都在 7.5 上下 —— **1-4 的宽度根本不由环扭转决定,这笔债不在这儿。**
//
// 真正的差距量出来了:RDKit 钉住 58.7% 的 1-4,而那**不是**靠立体标记 ——
// 实测语料里 9298 根键上只有 **51 根**带立体标记(0.55%),撑不起 59%。
// 它靠的是 **sp²–sp² 键**(芳环、共轭、酰胺,由共轭定平面),
// 见 `_setInRing14Bounds` 的 `preferCis`。我们目前只钉了芳**环**,
// 没钉一般的 sp²–sp² 键。**那才是下一步**,做完这条闸才有可能降回 1.02。
