...
mod tests {
    ...
    /// 解析 + 净化 + **顺反感知**。
    ///
    /// 第三步不能省:净化那 12 步里没有它,漏了的话每根双键的 `stereo` 都是
    /// `None`,顺反校正整个空转 —— 于是顺式和反式画成同一张图,而判据照样绿。
    /// [`crate::generate`] 的 `debug_assert!` 现在会当场拦住这种输入。
    ...
    fn no_drawn_line_runs_across_an_atom_label() {
        ...
        // 标签没有、或挨着"塞不下"的键的原子会被整个跳过 —— 整条判据一条线都
        // 没查到时它就是空过的
    ...
    fn a_fused_ring_double_bond_goes_inside_one_ring_not_astride_the_bond() {
        ...
        // **守卫放在最外面。** 单个分子上可以一根都没有(萘的合拢键在这个
        // Kekulé 式里是单键),但整条判据一根都没查到就是空过了。
    ...
    fn nothing_is_drawn_outside_the_canvas() {
        ...
        // **守卫放在最外面。** 苯整个分子没有一个标签(全是裸骨架碳),它在
        // 这条判据里验的是上半段的图元;下半段要整条判据都没查到才算空过。
