...
/// 取多轮最小值的轮数。
///
/// **与 `omgkit-chem/tests/scaling.rs` 对齐成 20。** 先前是 8,而那边的注释
/// 早就记着"轮数不够时,并行争用一下就能把某一档的几轮全抬高"。
/// 实测本机:8 轮时这个文件十次红一次(签名是**中间那档**偏高 ——
/// 699→1.31、1399→**1.95**、2799→1.43,最大档反而回落,不是平方项);
/// 20 轮时十次零次。耗时 0.12 → 0.62 秒。
///
/// 注意这与"失败了就重试"是两回事:重试是**只会让判据变绿**的单向过滤器
/// (那条路走过一次,量出来边缘缺陷的检出率从 47% 掉到 20%,已回退,见
/// 开发笔记)。加轮数不动判据规则,只是让 min-of-N 这个估计量更贴近真最小值 ——
/// 对健康值和缺陷值同样收紧,不牺牲任何检出力。
///
/// (样本只有 10 次对 10 次,不足以把"20 轮就不抽了"当结论;
///  真正的理由是同一套判据在同一个仓库里该用同一个轮数。)
