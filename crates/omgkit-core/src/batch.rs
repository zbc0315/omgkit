...
//! 这一个决策同时买到:
//!
//! 1. **CPU 速度** —— 顺序访存、可向量化、预取友好
//! 2. **GPU 就绪** —— 这些数组本身就是设备端布局,`memcpy` 整块过去,零编组
//! 3. **零拷贝** —— 列可直接暴露为 numpy / Arrow buffer
//! 4. **廉价并行** —— 按分子偏移切分,无共享状态
//!
//! # 索引约定
//!
//! 批内部一律使用**全局**下标(跨分子连续)。这是 CSR 与 GPU 传输的自然形式。
