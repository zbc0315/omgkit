...
pub struct AtomData {
    ...
    /// 追加到末尾),方括号里的氢也占一个配体位 —— 两件事都会让同一个几何构型
    /// 对应到不同的序号。所以"相对什么顺序"必须说清楚:
    ///
    /// | 类别 | 本字段相对什么顺序 |
    /// |---|---|
    /// | [`ChiralTag::SquarePlanar`] **且恰好 4 个邻居** | **邻居的存储顺序**(已归一) |
    /// | `SquarePlanar` 但邻居数不是 4 | 书写时的字面值(表达不出来,写出时整个丢掉) |
    /// | [`ChiralTag::TrigonalBipyramidal`] / [`ChiralTag::Octahedral`] / [`ChiralTag::Other`] | 书写时的字面值 |
    ///
    /// **平面四方能归一是因为它的序号就是"哪两对配体互为反位"** —— 换个配体
    /// 顺序只是换一个配对,不需要查找表。换算见
    /// [`square_planar_renumber`](crate::square_planar_renumber),那张对应关系是
    /// 从 RDKit 2025.09.2 穷举量出来的(72 种写法零反例)。
    ///
    /// 三角双锥与八面体不行:它们的序号枚举的是配体在多面体位置上的排法
    /// (模掉该多面体的转动群),邻居每做一次对换,序号按一张查找表跳到另一个值。
    /// 那张表尚未实现,所以那两类只是**原样保管**书写值,不声称它相对任何其它
    /// 顺序成立;写出时直接丢弃(判据:`roundtrip_smiles.rs` 里那条
    /// `trigonal_bipyramidal_octahedral_and_axial_stereo_are_not_written_yet`)。
    ///
    /// 缓一缓的理由是实测的:8839 条真实语料里含配位几何的分子 **0 条**。
    /// (同属立体化学的双键顺反已经做了,见 [`BondData::stereo`]。)
    ///
    /// 也因此,拿后两类与任何"已归一"的表示逐值比对都没有意义 —— 两者只在
    /// "没有重排且配体齐全"时才碰巧相等。
    pub stereo_perm: u8,
