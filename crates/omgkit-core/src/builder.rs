...
pub struct AtomData {
    ...
    /// # 这是**书写时的字面值**,尚未归一到存储序
    ///
    /// `@TB15` 存 15。序号的含义依赖"配体按什么顺序排列",而解析会重排邻居
    /// (环闭合键统一追加到末尾),方括号里的氢也占一个配体位 —— 两件事都会
    /// 让同一个几何构型对应到不同的序号。
    ///
    /// 把序号换算到存储序的参照系需要一张查找表:序号枚举的是配体在多面体
    /// 位置上的排法(模掉该多面体的转动群),邻居每做一次对换,序号就按表
    /// 跳到另一个值。这张表尚未实现,所以本字段只是**原样保管**书写值,
    /// 不声称它相对任何其它顺序成立;写出时这类标记直接丢弃。
    ///
    /// 不做的理由是实测的:8839 条真实语料里含配位几何的分子 **0 条**。
    /// (同属立体化学的双键顺反已经做了,见 [`BondData::stereo`]。)
    ///
    /// 也因此,拿这个字段与任何"已归一"的表示逐值比对都没有意义 —— 两者只在
    /// "没有重排且配体齐全"时才碰巧相等。
    pub stereo_perm: u8,
