...
pub struct AtomData {
    ...
    /// | 以上三类但**邻居数对不上** | — | 书写时的字面值;写出时整个丢掉 |
    /// | [`ChiralTag::Other`](`@AL` 等) | — | 书写时的字面值 |
    ///
    /// 邻居数对不上时不归一,是因为方括号里的氢也占一个配位位置而它不在邻居
    /// 序列里 —— 少了它换算出来的是另一个排法。那时写出侧**整个丢掉**这个标记:
    /// 丢掉是老实的,瞎写一个序号是撒谎。实测 RDKit 在 5 配体、1 配体的 `@SP` 上
    /// 同样整个丢掉。
    ///
