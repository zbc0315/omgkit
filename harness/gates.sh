...
# **基准与生成它的脚本脱钩了没有。** 只比结构,不比数值 ——
# 基准的数值各钉在各自的 RDKit 版本上(手性那份是 2022.09.5、界那份是 2025.09.2),
# 逐字节比会天天红;而结构与版本无关(实测:同一批脚本在 2022.09.5 与 2025.09.2
# 下结构逐个相同,只是 lonepair 那份的分子数 15 vs 16 —— 值会变,结构不会)。
#
