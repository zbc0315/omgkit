...
**这是个生成器,而且它产出的基准眼下没有消费者。** 现在守产物生成的是
`check_reactions.py`,它吃的是 `dump_reactions` 的输出、当场向外部实现求真值,
不读这里写的 `reactions.tsv`。留着是因为"预先烘好一份基准"那条路将来可能要
(全量 ChEMBL 规模上当场求真值太慢),但**在有人接它之前,它不是闸**。
