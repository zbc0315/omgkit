...
**分子内反应**目前也做不了:模板写成两个片段、而底物里这两个片段仍连在同一个
分子上时,`run` 那套"按位置配 N 个分子"的接口表达不了。USPTO-50k 上这类占
301 条(0.6%),RDKit 的 `RunReactants` 同样交白卷。
...
`RunReactants` 逐条对比耗时与命中。1288 条未命中**逐条归了因**,全部是记录或
模板本身的极限(其中 44 条经 rdchiral 裁定,它自己也还原不出)—— 在模板能表达
的范围内两个方向都是 100%。同一批数据里 RDKit 有 231 条产物**质量不守恒**。
数据、脚本、图与逐条证据在 [`benchmark-uspto-50k/`](benchmark-uspto-50k/)。
