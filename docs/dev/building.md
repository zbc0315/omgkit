...
覆盖率停在 **14.3%**(1259 / 8831),而 RDKit ETKDG 是 99.48%(2026-08-26 拿钉住的
2025.09.2 重量是 99.59%,见 `harness/baseline_rdkit_etkdg.py`)。
...
| **全语料硬不变量** | 8831 个分子跑完整条流水线:界可行率、原子完全重合、坐标非有限数 | 界不可行 **0.01%**(1 个;同一份语料上 RDKit ETKDGv3 2025.09.2 是 **0.41%**,36 个);重合 **0**;非有限 **0** |
| 手性中心 | 真值 = 真实构象上量出来的有符号体积 | 247 个中心,符号错 0、漏抽 0 |
| **端到端(产物)** | 精修前后各量一遍:越界、自穿、手性、耗时 | 1-2/1-3/1-4/长程 越界 **0.0 / 0.1 / 0.0 / 0.0%**;键交叉 1731 → **0**;手性 85.1% → **100%**;**1.45 ms/分子**(全语料口径 1.1 ms,参照 6.1 ms) |
| 自穿 | 先拿真实构象校准检测器(必须报 0),再量自己 | 真实构象 0 误报;我们嵌出来的环穿刺 1/400(0.2%),键交叉 1821 —— 后者正是精修要收拾的 |
| **通用性难例语料** | 68 个分子,照着算法的假设挑:笼状/张力、超配位、累积双键、超大环、少见元素、金属、自由基、两性离子 | 建界即空 0、界不可行 **0**。**这一档 RDKit 追上来了**:2026-08-26 拿钉住的 2025.09.2 重量,同一批分子它失败 **0 个** —— 先前记的"失败 2 个(SF₆ 与六氨合钴)"出自更早的版本 |
...
还没做:确定性的重试阶梯、1-5 链式约束。**已知的一笔债**:饱和环的
