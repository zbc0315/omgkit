...
本文记录 omgkit 各层的设计取舍,以及每一项的**差分验证结果**。面向的是要改这份
代码、或者想知道某个结论凭什么成立的人。只想用的话看 [README](../README.md) 就够。

差分测试那套判据(怎么写才不空过、覆盖面怎么算)另见
...
读者规范化再比多重集。当前 **717 组一致、24 组产物不同**,"只有一边给出产物"
两个方向都是 0。

那 24 组的差别来自一个**刻意的设计选择**:本库在 L1/L2 保留显式 `[H]` 原子
(`removeHs` 划在净化之外),而基准在解析时就把它合并掉了 —— 同一个底物在两侧
的图本来就不同,产物自然也不同。其中 10 组里基准给出的产物含净化不了的片段。
